...
ResidualBonus = {
  ...
  config: {
    matrix: [
      { //each setup is a configuration
        setup: [
          { //each array position represents a level of depth
            commission: 0.00
          }, {
            commission: 0.05
          }, {
        ...
        setup: [
          { //create additional setups to test different commissions
            commission: .00
          }, {
            commission: .10
          }, {
  ...
  calculate: {
    runReport: function(items) {
      ...
      var itemArray = ResidualBonus.helpers.sanitizeTree(ResidualBonus.helpers.CreateTreeFromArray(items));
  ...
  helpers: {
    ...
    setMaxPaymentDepthByVG: function(item) {
      ...
      var depthToPay = FittedRank.depth < currentRank.depth
        ? FittedRank.depth
        : currentRank.depth;
      //==================
